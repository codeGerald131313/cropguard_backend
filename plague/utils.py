...
def predict_plague(image_file):
    try:
        # Cargar el modelo desde la ruta configurada
        model = tf.keras.models.load_model(settings.MODEL_PATH)
        
        # Preprocesar la imagen
        img = Image.open(image_file).convert('RGB').resize((64, 64))  # Asegúrate de usar el tamaño correcto
        img_array = np.array(img)
        img_array = tf.image.resize(img_array, (64, 64))  # Asegúrate de usar el tamaño correcto
        ...
        class_name = class_names_dict[predicted_class_index]
        
        # Obtener la confianza (probabilidad) de la predicción
        confidence = np.max(predictions) * 100  # Ya en porcentaje
        
        return {
            'class': class_name,
            'confidence': float(confidence)  # Valor correcto ya en porcentaje
        }
